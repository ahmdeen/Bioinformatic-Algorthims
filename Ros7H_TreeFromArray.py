...
def TreeFromArray(text, suffixArray, lcp):

    sTree = SuffixArrayToTree(text, suffixArray, lcp)


    return [sTree.edge_word(edge) for edge in sTree.edges.values()]

with open('Data/rosalind_7h.txt') as infile:
	text = infile.readline().strip()
	suffixArray, lcp = [map(int,line.strip().split(', ')) for line in infile.readlines()]

answer = sorted(TreeFromArray(text,suffixArray, lcp))

print '\n'.join(answer)

with open('Ros7H_Answer.txt','w') as outfile:
	outfile.write('\n'.join(answer))